...
```{r, get data}
cts <- read.table("file:///C:/Users/rgoer/Documents/qPCR stuff/CAD ZBP1 endo genes2019-12-13 -  Quantification Cq Results_0.csv", header = TRUE, sep = ",")
...
cts_tidy %>% select(row, gene, Rescue, Fraction, rep, diff) %>% mutate(diff = -diff) %>% spread(Fraction, diff) %>% mutate(LR = neurite - soma) %>% filter(gene == "tsc1") %>% ggplot(aes(Rescue, 2**-LR, fill = Rescue)) + geom_boxplot() + geom_point(position = position_jitterdodge()) + theme_cowplot() + labs(y = "2^((ranbp1 - tsc1 neurite) / (ranbp1 - tsc1 soma))")
...
cts_tidy %>% filter(gene != "tsc1") %>% group_by(gene, Rescue, Fraction, rep) %>% summarise(mean_diff = mean(diff)) %>% spread(Fraction, mean_diff) %>% mutate(LR = neurite - soma) %>% ggplot(aes(Rescue, -LR, fill = Rescue)) + geom_boxplot() + geom_point(position = position_jitterdodge()) + theme_cowplot() + stat_compare_means(comparisons = comparisons) + facet_grid(. ~ gene)
